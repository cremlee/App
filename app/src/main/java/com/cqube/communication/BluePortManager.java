...
public class BluePortManager  {
    ...
    private BluePortManager(ICommunication iCommunication) {
        ...
        try {
            ...
            mReadThread = new BluePortManager.ReadThread();
            mReadThread.start();
    ...
    private static BluePortManager single = null;
     ...
     * @return
     */
    public static BluePortManager getInstance(ICommunication iCommunication) {
        if (single == null) {
            single = new BluePortManager(iCommunication);
        }
        return single;
    }
    /**
    ...
    public synchronized boolean write(byte[] cmd) {
        if (mOutputStream == null) {
            return false;
        }
        try {
    ...
    protected SerialPort getSerialPort() throws SecurityException, IOException, InvalidParameterException {
        if (mSerialPort == null) {
            mSerialPort = new SerialPort(new File("/dev/ttymxc2"), 115200, 0);
        }
        return mSerialPort;
    }
