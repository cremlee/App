package android.luna.ViewUi.SquareMenu;
...
public class SquareMenu extends View {
    ...
    private void setupAttributes(AttributeSet attrs) {
        ...
        float basevalue = (float)fabWidth;
        Matrix matrix = new Matrix();
        if(iconM1 != null) {
            bitmapM1 = ((BitmapDrawable) iconM1).getBitmap();
            matrix.setScale((float) (basevalue/bitmapM1.getWidth()), (float) (basevalue/bitmapM1.getHeight()));
            bitmapM1 = Bitmap.createBitmap(bitmapM1, 0, 0, bitmapM1.getWidth(),
                    bitmapM1.getHeight(), matrix, true);
        }
        if(iconM2 != null) {
            bitmapM2 = ((BitmapDrawable) iconM2).getBitmap();
            matrix.setScale(basevalue/bitmapM2.getWidth(), basevalue/bitmapM2.getHeight());
            bitmapM2 = Bitmap.createBitmap(bitmapM2, 0, 0, bitmapM2.getWidth(),
                    bitmapM2.getHeight(), matrix, true);
        }
        if(iconM3 != null) {
            bitmapM3 = ((BitmapDrawable) iconM3).getBitmap();
            matrix.setScale(basevalue/bitmapM3.getWidth(), basevalue/bitmapM3.getHeight());
            bitmapM3 = Bitmap.createBitmap(bitmapM3, 0, 0, bitmapM3.getWidth(),
                    bitmapM3.getHeight(), matrix, true);
        }
    }
    ...
    private void setupPaint() {
        ...
        paintFabBGWidShadow.setColor(Color.RED);
    ...
    protected void onDraw(Canvas canvas) {
       ...
       // Draw Squares
        ...
        else if(menuOpenDirection.equals(TOP_RIGHT))
            drawSquares4TopRightMenu(canvas);
        else if(menuOpenDirection.equals(BOTTOM_RIGHT))
            drawSquares4BottomRightMenu(canvas);
    }
    ...
    private void drawSquares4BottomRightMenu(Canvas canvas){

        // Draw BG Rect
        canvas.drawRect(0,0, fabHeight, fabWidth, paintFabBG);
        ...
        // Draw PLUS
        drawPlusIcon(canvas,  FAB_ICON_PADDING, fabHeight*0.5f,  fabWidth-(2*FAB_ICON_PADDING), plusRotationDegree);

        // Draw Menus icon
        if( isOpened ) drawBottomRightMenuIcons(canvas);
    }
    ...
    private void drawTopLeftMenuIcons(Canvas canvas){
        ...
        if(bitmapM3 !=null) {
            imageStartX = (fabWidth + (fabWidth/2)) - (bitmapM3.getWidth()/2);
            imageStartY = (0 + (fabHeight/2)) - (bitmapM3.getHeight()/2);
    ...
    private  void drawBottomRightMenuIcons(Canvas canvas){
        int imageStartX,imageStartY;
        if(bitmapM3 !=null) {
            imageStartX = (0 + (fabWidth/2)) - (bitmapM3.getWidth()/2);
            imageStartY = (fabHeight + (fabWidth/2)) - (bitmapM3.getHeight()/2);
            canvas.drawBitmap(bitmapM3, imageStartX , imageStartY , paintIcons);
        }

        if(bitmapM1 !=null) {
            imageStartX = (fabWidth + (fabWidth/2)) - (bitmapM1.getWidth()/2);
            imageStartY = (0 + (fabHeight/2)) - (bitmapM1.getHeight()/2);
            canvas.drawBitmap(bitmapM1, imageStartX , imageStartY, paintIcons);
        }

        if(bitmapM2 !=null) {
            imageStartX = (0 + (fabWidth*3/2)) - (bitmapM2.getWidth()/2);
            imageStartY = (0 + (fabHeight*3/2)) - (bitmapM2.getHeight()/2);
            canvas.drawBitmap(bitmapM2, imageStartX , imageStartY, paintIcons);
        }
    }
    @Override
    ...
    private void touch4BottomRightMenu(MotionEvent event) {
        if(!isOpened) {
    ...
    public  void SetLanguage(int[] drawableres)
    {
        float basevalue = (float)fabWidth;
        Matrix matrix = new Matrix();
        if(drawableres[0] != 0) {
            bitmapM1 = ((BitmapDrawable) getResources().getDrawable(drawableres[0])).getBitmap();
            matrix.setScale((float) (basevalue/bitmapM1.getWidth()), (float) (basevalue/bitmapM1.getHeight()));
            bitmapM1 = Bitmap.createBitmap(bitmapM1, 0, 0, bitmapM1.getWidth(),
                    bitmapM1.getHeight(), matrix, true);
        }
        if(drawableres[1] != 0) {
            bitmapM2 = ((BitmapDrawable) getResources().getDrawable(drawableres[1])).getBitmap();
            matrix.setScale(basevalue/bitmapM2.getWidth(), basevalue/bitmapM2.getHeight());
            bitmapM2 = Bitmap.createBitmap(bitmapM2, 0, 0, bitmapM2.getWidth(),
                    bitmapM2.getHeight(), matrix, true);
        }
        if(drawableres[2] != 0) {
            bitmapM3 = ((BitmapDrawable) getResources().getDrawable(drawableres[2])).getBitmap();
            matrix.setScale(basevalue/bitmapM3.getWidth(), basevalue/bitmapM3.getHeight());
            bitmapM3 = Bitmap.createBitmap(bitmapM3, 0, 0, bitmapM3.getWidth(),
                    bitmapM3.getHeight(), matrix, true);
        }
