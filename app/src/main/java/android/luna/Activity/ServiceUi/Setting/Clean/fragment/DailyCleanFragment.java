...
public class DailyCleanFragment extends  CleanFragment implements View.OnClickListener {
    private String cmd="";
    @Override
    public void InitView(View view) {
        super.InitView(view);
        setTitle("Daily Clean");
        setIcon(R.mipmap.clean_daily_care);
        setContent(getActivity().getString(R.string.clean_daily_cxt));
        getStart().setOnClickListener(this);
        try {
            cmd = new CmdCleanMachine().buildCmdStart(DeviceXmlFactory.getCleanComponent(FileHelper.FILE_DEVICE_CONFIG,DeviceXmlFactory.CLEAN_TYPE_DAILY));
        }
        catch (Exception e){}
    }
