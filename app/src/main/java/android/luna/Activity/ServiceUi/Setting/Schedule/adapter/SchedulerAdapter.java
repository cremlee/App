...
public class SchedulerAdapter extends BaseAdapter {
	...
	private Map<Integer, List<SchedulerDetail>> mapSchedulerDetails;

	private int startColor, endColor, normalColor;

	public SchedulerAdapter(Context context, Map<Integer, List<SchedulerDetail>> schedulerDetails,ScheduleDaoFactory scheduleDaoFactory) {
	...
	@Override
	public List<SchedulerDetail> getItem(int position) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (mapSchedulerDetails != null) {
			List<SchedulerDetail> schedulerDetails = mapSchedulerDetails.get(position);
			if (schedulerDetails != null) {
				int size = schedulerDetails.size();

				for (int i = 0; i < size; i++) {
					SchedulerDetail detail = schedulerDetails.get(i);
					...
					if (persistFlag == 1) {
						color = startColor;
						 ...
						 //color = endColor;
						 name = context.getString(R.string.sch_wakeup);//"Wake Up";
						 }
	...
	public OnClickListener clickListener(final int hour, final int week) {
		return new OnClickListener() {
			...
			public void onClick(View view) {
				SchedulerDetail detail = scheduleDaoFactory.getScheduleDetailDao().findSchedulerDetailByWeekAndHour(week, hour);
				int schedulerId = 0;
				if (detail != null) {
